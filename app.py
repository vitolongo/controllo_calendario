"""
MOHOLE - Dashboard Controllo Ore Importazione Calendario
...
def check_password():
    def password_entered():
        if st.session_state["password"] == "Sperem2025!!":  # Cambia questa password
...
# ========== CONFIGURAZIONE ==========
st.set_page_config(
    page_title="Mohole - Controllo Ore Importazione Calendario",
    page_icon="📊",
    layout="wide",
...
def parse_time(x):
    ...
    # Prova frazione di giorno (Excel)
    try:
        f = float(s)
        if 0 <= f < 2:
            minutes = f * 24 * 60
...
def check_duplicates(df):
    ...
    key_cols = ['_date', '_start_time', '_end_time', 'SEDE', '_cf_norm']
    
    # Converti time a stringa per groupby
    df_work = df.copy()
...
def check_overlaps(df):
    ...
    for (date_key, cf), g in work.groupby(['_date_str', '_cf_norm']):
        ...
        active = []  # (end_time, row_idx, start_time)
